...
if __name__ == '__main__':
    seg_path = '/data/cbam/pancreas'
    gd_path = '/data/DATASET3/nnUNet_raw/nnUNet_raw_data/Task007_Pancreas/labelsTs'
    save_dir = '/data/cbam/pancreas'
    ...
    for name in seg:
        if not name.startswith('.') and name.endswith('nii.gz'):
            ...
            seg_arr = seg_.get_fdata().astype('float32') == 1
            gd_ = nib.load(os.path.join(gd_path, name))
            gd_arr = gd_.get_fdata().astype('float32') == 1
            ...
            # 求IOU
            iou_score = binary_iou(seg_arr, gd_arr)
            ...
            # 求dice
            dice = get_evaluation_score(seg_.get_fdata() == 2, gd_.get_fdata() == 2, spacing=None, metric='dice')
            dices.append(dice)
            ...
            specs.append(spec)
    # 存入pandas
    data = {'dice': dices, 'RVE': rves, 'Sens': senss, 'Spec': specs, 'HD95': hds, 'IOU': iou_scores}
    df = pd.DataFrame(data=data, columns=['dice', 'RVE', 'Sens', 'Spec', 'HD95', 'IOU'], index=case_name)
